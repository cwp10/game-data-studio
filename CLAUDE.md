...
| 2026-07-07 | 하네스 감사: ①00_genre_contract 죽은 참조 8곳 제거(genres/*.md를 장르별 SSOT로 승격) ②ui-builder·gds-ui 본문 5→8개 화면 갱신(경제·타입·메모리 경로/레이아웃/Sidebar 순서 실코드 반영) ③qa-validator 호출을 커스텀 타입 직접 호출로 통일 ④_workspace 실경로·기능코드({F}) 네이밍 명시 ⑤game-schema description RPG 6종 키워드 보강 | agents/(schema-designer·ui-builder·qa-validator·balance-engineer), skills/(game-schema·game-data-feature) | 정기 감사에서 drift 발견 — 실코드 8화면 대비 정의 낡음, 존재하지 않는 SSOT 참조 |
