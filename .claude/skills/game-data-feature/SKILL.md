...
> `_workspace/`의 실제 경로는 `.claude/skills/game-data-feature/_workspace/`다. 이하 상대 표기.
> 산출물 파일명은 기능 코드 접두 `{F}`를 사용한다 (예: `F1_contract.md`, `F1_stageA_spec.md`).
...
2. 요청 유형 분류:
   ...
   - `help` — 도움말 팝업(HelpModal) 생성·수정 → **help-builder 에이전트** 단독 호출
...
balance-engineer 에이전트 (model: "opus")
  ...
  → 출력:
      _workspace/{F}_stageA_spec.md  — 결정적 앵커 + shape 정의
      _workspace/{F}_contract.md     — Stage B shape 계약 (단일 출처)
      _workspace/{F}_stageA_summary.md
```

**Stage A 완료 후 Stage B 진입 (게이트 통과 시만)**
```
mcp-implementer (background) + ui-builder (background) 병렬 실행
  각자 _workspace/{F}_contract.md를 단일 출처로 소비
  → mcp-implementer: API route action 추가 + MCP 핸들러
  → ui-builder: SimulationView 유형 추가
  → 출력: _workspace/{F}_stageB_summary.md
```
...
### help 요청
```
help-builder 에이전트 (model: "opus") 단독 호출
  → help-content 스킬 + gds-ui 스킬 활용
  → 신규: HelpModal.tsx 구현 + Sidebar ? 버튼 연결
  → 수정: 해당 화면 섹션만 교체
```
...
```
qa-validator 에이전트 (model: "opus", subagent_type: "qa-validator")
  입력:
    - scope: 구현한 기능명
